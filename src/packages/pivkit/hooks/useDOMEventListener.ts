...
import { EventListenerController } from '../../domkit/gesture/addEventListener'
...
export function useDOMEventListener<El extends ElementAccessors, K extends keyof HTMLElementEventMap>(
  ...
  createEffect(() => {
    ...
    els.forEach((el) => {
      ...
      const { cancel } = onEvent(el, eventName, fn, options)
      onCleanup(cancel)
    })
  })
}
...
export function useDocumentEventListener<K extends keyof HTMLElementEventMap>(
  ...
  createEffect(() => {
    // @ts-expect-error no need to check
    const { cancel } = onEvent(globalThis.document, eventName, fn, options)
    onCleanup(cancel)
  })
}
...
export function useWindowEventListener<K extends keyof HTMLElementEventMap>(
  ...
  createEffect(() => {
    // @ts-expect-error no need to check
    const { cancel } = onEvent(globalThis.window, eventName, fn, options)
    onCleanup(cancel)
  })
}
