import { getNow, getTimestamp, isCurrentDateAfter, switchCase, type MayPromise } from "@edsolater/fnkit"
      ...
      res
        ...
        .then((buffer) => {
          ...
          getResponseCache(config.storePlace).set(key, {
            ...
            ok: true,
            expireAt: getNow() + config.shelfLife,
          })
        })
    }
    return res
...
async function getRecordedResponse(config: {
  ...
  if (cacheItem.expireAt != null && isCurrentDateAfter(cacheItem.expireAt)) {
